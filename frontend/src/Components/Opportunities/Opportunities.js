...
function Opportunities(props) {
  ...
  function getFirstThree() {
    for (let i = 0; i < opportunities.length; i++) {
      if (
        opportunities[i]._id === "6099c78c001ee300081c1dab" ||
        opportunities[i].title === "Board Member"
      ) {
    ...
    for (let j = 0; j < opportunities.length; j++) {
      if (
        moment
          .duration(
            moment(
              opportunities[j].end_event[opportunities[j].end_event.length - 1]
            ).diff(moment().startOf("day"))
          )
          .asHours() > 0
      ) {
        results.push(opportunities[j]);
      }
      if (results.length === 3) {
        break;
      }
    ...
    <div>
      ...
      <div className="oppDisplay">
        <Row className="justify-content-md-center">
          ...
          <Col md="auto">
            <button onClick={navigateTo} className="viewMore">
