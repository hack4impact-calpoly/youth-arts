...
function UpcomingOpportunities(props) {
  ...
  function handleButtonPress(index) {
    const updated = upcomingOpportunities.filter((opp, i) => i !== index);
    setUpOpps(updated);
    return true;
  }
          ...
          <Row key={i} className="upOpp">
            <Col md="auto" className="col-6">
              <Link to={`/opportunityDetail/${opp.id}`} className="pastOppLink">
                ...
                <p className="upOppDate">
                  {`${Moment(opp.start)
                    .tz("America/Los_Angeles")
                    .format("MMMM Do YYYY, h:mm a")} to ${Moment(opp.end)
                    .tz("America/Los_Angeles")
            ...
            <Col className="upOppCancel">
              <Button
                variant="danger"
                onClick={() => handleButtonPress(i) && props.handleCancel(opp)}
              >
                Cancel
              </Button>
            </Col>
