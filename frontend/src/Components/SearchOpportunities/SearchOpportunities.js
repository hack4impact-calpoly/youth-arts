...
function SearchOpportunities(props) {
    ...
    fetchAll().then((result) => {
      if (result) {
        ...
        result = result.filter((opp) => {
          let diff = moment
            .duration(
              moment(opp.end_event[opp.end_event.length - 1]).diff(
                moment().startOf("day")
              )
            )
            .asHours();
        ...
        setOpportunities(
          result.sort((a, b) => (a.start_event < b.start_event ? -1 : 1))
        );
      }
    });
  }, []);
  ...
  // search logic
  let filteredOpps = Object.values(opportunities).filter((opportunity) =>
    opportunity.title.toLowerCase().includes(search.toLowerCase())
  );

  // sort logic
  if (sortBy === "oppAlphabetical") {
    filteredOpps = filteredOpps.sort((a, b) => a.title.localeCompare(b.title));
  }
  if (sortBy === "oppType") {
    filteredOpps = filteredOpps.sort((a, b) =>
      a.skills[0].localeCompare(b.skills[0])
    );
  } else if (sortBy === "date") {
    filteredOpps = filteredOpps.sort((a, b) =>
  ...
  if (filterBy === "Classroom") {
    filteredOpps = filteredOpps.filter((opportunity) =>
      opportunity.skills.includes("Classroom")
    );
  } else if (filterBy === "Event") {
    filteredOpps = filteredOpps.filter((opportunity) =>
      opportunity.skills.includes("Event")
    );
  } else if (filterBy === "Fundraiser") {
    filteredOpps = filteredOpps.filter((opportunity) =>
      opportunity.skills.includes("Fundraiser")
    );
  } else if (filterBy === "Maintenance") {
    filteredOpps = filteredOpps.filter((opportunity) =>
      opportunity.skills.includes("Maintenance")
    );
  } else if (filterBy === "Office/Admin") {
    filteredOpps = filteredOpps.filter((opportunity) =>
      opportunity.skills.includes("Office/Admin")
    );
  } else if (filterBy === "Performance") {
    filteredOpps = filteredOpps.filter((opportunity) =>
      opportunity.skills.includes("Performance")
    );
  }
