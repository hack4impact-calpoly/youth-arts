                ...
                return (
                    <button
                        ...
                        onClick={(e) => {
                            ...
                            let rowOpportunity = opportunities.filter(
                                (o) =>
                                    o.title === thisRow.title &&
                                    o.location === thisRow.location
                            );
                            let rowVols = [];
                            if (
                                ...
                                rowVols = Object.keys(
                                    rowOpportunity[0].volunteers
                                );
                                rowVols.forEach(function (
                                    part,
                                    index,
                                    theArray
                                ) {
                                    let curVol = volunteers.filter(
                                        (v) => v._id === part
                                    );
                                    if (
                                        curVol.length &&
                                        ...
                                        let taskList = [];
                                        Object.keys(
                                            curVol[0].opportunities
                                        ).map(function (key, index) {
                                            let tasks =
                                                curVol[0].opportunities[key];
                                            let times = tasks.map((task) => {
                                                ...
                                                for (
                                                    var i = 0;
                                                    i < task.donated.length;
                                                    i++
                                                ) {
                                                    donated = donated.concat(
                                                        task.donated
                                                    );
                                                }
                                            });
            ...
            valueGetter: ({ value }) => {
                if (Array.isArray(value)) {
                    return value
                        .map(
                            (item) =>
                                dateFormat(item, " mmmm dS, yyyy ") +
                                "at " +
                                dateFormat(item, "hh:MM TT")
                        )
                        .join(", \n");
                } else {
                    return value;
